...
def cross_entropy_loss(probs, target_index):
    ...
    # Your final implementation shouldn't have any loops
    # print(np.log(probs))
    return -sum(target_index * np.log(probs))
...
def linear_softmax(X, W, target_index):
    ...
    dW = np.dot(X.T, dpred)
    #print("predictions", predictions, "dW", dW, "loss", loss)
...
class LinearSoftmaxClassifier():
    ...
    def fit(self, X, y, batch_size=100, learning_rate=1e-7, reg=1e-5,
        ...
        for epoch in range(epochs):
            ...
            for batch in batches_indices:
                ...
                #print(xb, target_index)
                loss, dW = linear_softmax(xb, self.W, target_index)
                l2_loss, l2_dW = l2_regularization(self.W, reg)
                #print(l2_dW)
                self.W = self.W - learning_rate*dW - l2_dW
                #print(self.W.max(), dW.min(), l2_dW.max())
                #print(self.W)
    ...
    def predict(self, X):
        '''
        Produces classifier predictions on the setl2_dW
